...
struct Frame {
  ...
  Frame() = delete;
  virtual htm_t<T> get_htm() const = 0;
  virtual void set_htm(const htm_t<T>& htm) = 0;
  virtual vector_t<T, 3> get_position() const = 0;
  virtual void set_position(const vector_t<T, 3>& position) = 0;
  virtual quaternion_t<T> get_quaternion() const = 0;
  virtual void set_quaternion(const quaternion_t<T>& quaternion) = 0;
  virtual Frame<T, ALIGNMENT> operator*(const Frame& frame) const = 0;
  virtual Frame<T, ALIGNMENT> operator*(const htm_t<T>& htm) const = 0;
  virtual Frame<T, ALIGNMENT> inverted() const = 0;
  virtual void invert() = 0;
  virtual Frame<T, ALIGNMENT> resolve_in_frame(const Frame<T, ALIGNMENT>& frame) const = 0;
  virtual Frame<T, ALIGNMENT> resolve_in_frame(const htm_t<T>& htm) const = 0;
  virtual Frame<T, ALIGNMENT> operator()(const Frame<T, ALIGNMENT>& frame) const = 0; // resolve in frame
  virtual Frame<T, ALIGNMENT> operator()(const htm_t<T>& htm) const = 0; // resolve in frame
  virtual Frame<T, ALIGNMENT> operator()() const = 0; // resolve in global frame
};

template <typename T, uint32_t ALIGNMENT>
struct RelativeFrame : public Frame<T, ALIGNMENT>{
  using value_type = T;
  virtual Frame<T, ALIGNMENT>* get_parent() const = 0;
  virtual void set_parent(Frame<T, ALIGNMENT>* parent) = 0;
  virtual void set_parent(Frame<T, ALIGNMENT>& parent) = 0;
};
