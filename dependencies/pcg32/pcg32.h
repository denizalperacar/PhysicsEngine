#ifndef A27BD532_19B6_43C9_AD79_4D74DFCD85A2
#define A27BD532_19B6_43C9_AD79_4D74DFCD85A2
 ...
 *       It is uppdated here to work with the PhysicsEngine.
 */
...
struct pcg32 {
	...
	PE_HOST_DEVICE pcg32() : state(PCG32_DEFAULT_STATE), inc(PCG32_DEFAULT_STREAM) {}

	/// Initialize the pseudorandom number generator with the \ref seed() function
	PE_HOST_DEVICE pcg32(uint64_t initstate, uint64_t initseq = 1u) { seed(initstate, initseq); }
	 ...
	 */
	PE_HOST_DEVICE void seed(uint64_t initstate, uint64_t initseq = 1) {
		state = 0U;
		inc = (initseq << 1u) | 1u;
		next_uint();
		state += initstate;
	...
	PE_HOST_DEVICE uint32_t next_uint() {
	...
	PE_HOST_DEVICE uint32_t next_uint(uint32_t bound) {
	...
	PE_HOST_DEVICE float next_float() {
	 ...
	 */
	PE_HOST_DEVICE double next_double() {
		/* Trick from MTGP: generate an uniformly distributed
	 ...
	 * are made inbetween.
	 */
	PE_HOST_DEVICE void advance(int64_t delta_ = (1ll<<32)) {
		uint64_t
			cur_mult = PCG32_MULT,
			cur_plus = inc,
			acc_mult = 1u,
	...
	PE_HOST_DEVICE int64_t operator-(const pcg32 &other) const {
		uint64_t
			cur_mult = PCG32_MULT,
			cur_plus = inc,
	...
	/// Equality operator
	PE_HOST_DEVICE bool operator==(const pcg32 &other) const { return state == other.state && inc == other.inc; }
...
#endif /* A27BD532_19B6_43C9_AD79_4D74DFCD85A2 */
